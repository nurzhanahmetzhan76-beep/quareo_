/* ============================================================
   Quareo — shared i18n + common behaviors + auth
   ============================================================ */
...
const RP_I18N = {
  ru: {
    ...
    footer_l_scanner: "Niche Scanner", footer_l_wb: "Сканер WB", footer_l_analytics: "Аналитика", footer_l_ntin: "NTIN", footer_l_waybills: "Накладные", footer_l_pricing: "Тарифы",
  ...
  en: {
    ...
    footer_l_scanner: "Niche Scanner", footer_l_wb: "WB Scanner", footer_l_analytics: "Analytics", footer_l_ntin: "NTIN", footer_l_waybills: "Waybills", footer_l_pricing: "Pricing",
...
(function () {
  ...
  if (!document.getElementById(STYLE_ID)) {
    ...
    style.textContent = `
      ...
      .rp-toast {
        pointer-events: auto;
        position: relative;
        background: linear-gradient(135deg, rgba(255,255,255,0.55), rgba(219,234,254,0.45));
        ...
        gap: 10px;
        animation: rpToastIn .35s cubic-bezier(.22,1,.36,1);
      ...
      .rp-toast.rp-toast-error {
        background: linear-gradient(135deg, rgba(255,255,255,0.55), rgba(254,226,226,0.55));
        box-shadow: 0 1px 2px rgba(220,38,38,.06), 0 12px 28px rgba(220,38,38,.18), inset 0 1px 0 rgba(255,255,255,.9);
      }
      .rp-toast.rp-toast-success {
        background: linear-gradient(135deg, rgba(255,255,255,0.55), rgba(220,252,231,0.55));
        box-shadow: 0 1px 2px rgba(22,163,74,.06), 0 12px 28px rgba(22,163,74,.18), inset 0 1px 0 rgba(255,255,255,.9);
      }
      .rp-toast-icon {
        flex-shrink: 0;
        width: 26px;
      ...
      .rp-toast-error .rp-toast-icon { background: rgba(220, 38, 38, 0.15); }
      .rp-toast-success .rp-toast-icon { background: rgba(22, 163, 74, 0.15); }
  ...
  /**
   * Shows a styled toast notification instead of the native alert().
   * @param {string} message
   * @param {'info'|'success'|'error'} type
   * @param {number} duration ms before auto-dismiss (0 = stay until closed)
   */
  window.showToast = function (message, type = 'info', duration = 4500) {
    const container = getContainer();

    // Avoid stacking identical messages (e.g. from rapid double-clicks)
    const existing = Array.from(container.querySelectorAll('.rp-toast'))
      .find(t => t.dataset.msg === message && t.classList.contains(`rp-toast-${type}`));
    if (existing) {
      clearTimeout(existing._rpTimer);
      if (duration > 0) existing._rpTimer = setTimeout(() => {
        existing.classList.add('rp-toast-out');
    ...
    if (duration > 0) el._rpTimer = setTimeout(remove, duration);
  };

  /**
   * Drop-in replacement for window.alert() with nicer styling.
