...
def upgrade() -> None:
    ...
    These tables may already exist if the app's Base.metadata.create_all()
    ran before this migration (e.g. via retailpool.bootstrap_db) — guard
    every create_table/enum-create so this migration is safe to run either
    before or after that bootstrap step.
    """
    conn = op.get_bind()
    ...
    # ── Niche Analyses ────────────────────────────────────────────────
    if "niche_analyses" not in tables:
        op.create_table(
            "niche_analyses",
            sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
    ...
    # Already created above (checkfirst-guarded) — don't let it also try to
    # auto-(re)create as a side effect of being embedded in a table below.
    pool_status_enum.create_type = False
    ...
    # ── Pool Participants ─────────────────────────────────────────────
    if "pool_participants" not in tables:
        op.create_table(
            "pool_participants",
            sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
